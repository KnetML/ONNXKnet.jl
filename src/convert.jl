...
"""
function convert_model(model::ONNXKnet.Proto.OperatorSetIdProto)
    a = Dict{Symbol, Any}()
...
"""
function convert_model(model::ONNXKnet.Proto.StringStringEntryProto)
    a = Dict{Symbol, Any}()
    fields = [:key, :value]
    for ele in fields
        a[ele] = getfield(model, ele)
...
function load_weights(name)
    ...
    for ele in keys(a)
        weights[string(ele)] = Float64.(a[ele])
    end
    return weights
end
...
function write_julia_file(model_file)
    f = readproto(open(model_file), ONNXKnet.Proto.ModelProto())
    data = ONNXKnet.code(convert(f).graph)
    touch("model.jl")
